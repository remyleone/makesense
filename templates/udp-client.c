...
void print_parents()
{
  ...
  for(nbr = nbr_table_head(ds6_neighbors); nbr != NULL; nbr = nbr_table_next(ds6_neighbors, nbr)) {
    ...
    printf("Neighbor %d\n", (int) ((&nbr->ipaddr)->u8[sizeof(uip_ipaddr_t)-1]));
...
send_packet(void *ptr)
{
  ...
  seq_id++;
  sprintf(buf, "%*d", (int) SIZE, seq_id);
  PRINTF("DATA send to root \'%s\'\n", buf);
...
PROCESS_THREAD(udp_client_process, ev, data)
{
  ...
  random_init(node_id);
  int random_backoff = random_rand() / 1000;
  PRINTF("RANDOM BACKOFF %d\n", random_backoff);
#if CBR_RATE > 0
  PRINTF("CBR RATE %d\n", (uint16_t) CBR_RATE);
  static struct stimer udp_cbr_timer;
  stimer_set(&udp_cbr_timer, (uint16_t) CBR_RATE + random_backoff);
#endif

#if POISSON_RATE > 0
  PRINTF("POISSON RATE %d\n", (uint16_t) POISSON_RATE);
  static struct stimer udp_poisson_timer;
  stimer_set(&udp_poisson_timer, poisson_time(POISSON_RATE) + random_backoff);
#endif
  ...
  while(1) {
...
#if POISSON_RATE > 0
    if(stimer_expired(&udp_poisson_timer) && parent_ok > 0) {
      send_packet(NULL);
...
#if CBR_RATE > 0
    if(stimer_expired(&udp_cbr_timer) && parent_ok > 0) {
      send_packet(NULL);
    ...
    if(stimer_expired(&rplinfo_timer)) {
      parent_ok = (int) rpl_get_parent_ipaddr((rpl_parent_t *) rpl_get_any_dag()->preferred_parent)->u8[sizeof(uip_ipaddr_t)-1];
...
#if BATTERY_RATE > 0
    if(stimer_expired(&battery_timer) && parent_ok > 0) {
