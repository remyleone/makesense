...
def load_estimation(path):
    ...
    if path:
        ...
        res = {
            "tx_sender_reference": lambda x: repeat_sender * float(8 * on_wire(x)) / rate,
            "rx_sender_reference": lambda x: repeat_receiver * ack_time,
            "tx_receiver_reference": lambda x: repeat_sender * ack_time,
            "rx_receiver_reference": lambda x: repeat_receiver * float(8 * on_wire(x)) / rate
        }
        return res
...
def message(folder):
    ...
    overhearing_regexp = r" ".join(
        [time_regexp, mote_id_regexp, "contikimac: data not for us"])

    mac_reception_regexp = r" ".join(
        [time_regexp, mote_id_regexp, "contikimac: data"])
    ...
    with open(pj(folder, "serial.log")) as serial_file:
        for line in serial_file:
            ...
            overhearing_match = re.match(
                overhearing_regexp, line, re.MULTILINE)
            if overhearing_match:
                d = overhearing_match.groupdict()
                d = cast_message(d)

                d["message_type"] = "overhearing"
                messages.add(message_t(**d))
                count_stats["overhearing", d["mote_id"]] += 1

            mac_reception_match = re.match(
                mac_reception_regexp, line, re.MULTILINE)
            if mac_reception_match:
                d = mac_reception_match.groupdict()
                d = cast_message(d)

                d["message_type"] = "mac_reception"
                messages.add(message_t(**d))
                count_stats["reception", d["mote_id"]] += 1
            ...
            battery_recalibration_match = re.match(
                battery_recalibration_regexp, line, re.MULTILINE)
    ...
    for name, count in sorted(count_stats.items()):
        if count:
...
class Estimator(object):
    ...
    def noinfo_estimation(self, current_message):
        ...
        self.bin_message_data_count[
            bin_start, current_message.node, "noinfo"] += 1
    ...
    def route_estimation(self, message):
        ...
        try:
            ...
            for forwarder in forwarders[1: -1]:
                ...
                t[forwarder, "tx"] += self.tx_sender(
                    size) + self.tx_receiver(size)
                t[forwarder, "rx"] += self.rx_sender(
                    size) + self.rx_receiver(size)
    ...
    def update_estimator(self, estimator, target, tx=0.0, rx=0.0, time=None):
        ...
        """
        if tx or rx:
            self.total_estimation[target, estimator, "tx"] += tx
            self.total_estimation[target, estimator, "rx"] += rx

            self.series_estimation[target, estimator, "time"].append(time)
            self.series_estimation[target, estimator, "tx"].append(self.total_estimation[target, estimator, "tx"])
            self.series_estimation[target, estimator, "rx"].append(self.total_estimation[target, estimator, "rx"])
    ...
    def handle_battery_recalibration_message(self, message, *args, **kwargs):
        ...
        if self.recalibration:
            ...
            t = message.time
            reality = {
                "tx": self.last_powertracker[node, "tx"],
                "rx": self.last_powertracker[node, "rx"]
            }
            self.last_known_powertracker[node].append((t, reality["tx"], reality["rx"]))
            ...
            for estimator in estimators:
                ...
                diff = {
                    "tx": reality["tx"] - self.total_estimation[node, estimator, "tx"],
                    "rx": reality["rx"] - self.total_estimation[node, estimator, "rx"]
                }
                self.update_estimator(estimator, node, time=t,
                                      tx=diff["tx"], rx=diff["rx"])
    ...
    def handle_energy_message(self, message, *args, **kwargs):
        ...
        # if self.recalibration:
        #     outcast_tx, outcast_rx = outcast(
        #         self.last_known_powertracker[current_node],
        #         current_t)
    ...
    def check(self, target):
        self.check_estimator_order(target)
        self.check_estimators_starts_with_message(target)
        self.check_model_trustworthy(target)

    def check_model_trustworthy(self, target):
        """
        Give some common values for tx and rx
        """
        log.info("Start check_model_trustworthy")
        for (
            bin_start, estimator) in itertools.product(self.bin_estimation["bins"],
                                                       estimators):
            ...
            try:
                ...
                log.info("----")
                log.info("Modeled cost per message (tx, sender): %f" %
                         self.tx_sender(avg_size))
                log.info("Modeled cost per message (rx, sender): %f" %
                         self.rx_sender(avg_size))
                log.info("Modeled cost per message (tx, received): %f" %
                         self.tx_receiver(avg_size))
                log.info("Modeled cost per message (rx, received): %f" %
                         self.rx_receiver(avg_size))
    ...
    def check_estimators_starts_with_message(self, target):
        ...
        for estimator in estimators:
            try:
                estimation_start = self.series_estimation[
                    target, estimator, "time"][0]
                log.info("Estimation for node %d starts at %f" %
                         (target, estimation_start))
            except IndexError as e:
                log.critical("Problem for: %d, %s" % (target, estimator))

    def check_estimator_order(self, target):
        """
    ...
    def save(self, target):
        ...
        path = pj(self.folder, "estimation",
...
def plot_series(folder, target):
    """
    :param powertracker_csv:
    :param output:
    """
    estimation_folder = pj(folder, "estimation")
    # powertracker values
    powertracker_csv = pj(estimation_folder,
                          "series_powertracker_%d.csv" % target)
    p_time, p_tx, p_rx = [], [], []

    with open(powertracker_csv) as f:
        reader = DictReader(f)
        for row in reader:
            row = {key: float(value) for key, value in row.items()}
            p_time.append(row["time"])
            p_tx.append(row["tx"])
            p_rx.append(row["rx"])
    ...
    for estimator in estimators:
        ...
        # estimated values
        estimator_csv = pj(estimation_folder,
                           "series_estimation_%d_%s.csv" % (target, estimator))
        e_time, e_tx, e_rx = [], [], []

        with open(estimator_csv) as f:
            reader = DictReader(f)
            for row in reader:
                row = {key: float(value) for key, value in row.items()}
                e_time.append(row["time"])
                e_tx.append(row["tx"])
                e_rx.append(row["rx"])
        ...
        ax1.set_title('Cumulative estimation {estimator} {target}/ real'.format(
            estimator=estimator, target=target))
        ax1.set_xlabel('Time (s)')
        ax1.set_ylabel('Time (s)')

        ax1.plot(p_time, p_tx, color="red", label="tx")
        ax1.plot(p_time, p_rx, color="blue", label="rx")

        ax1.plot(e_time, e_tx,
                 color="magenta", label="tx_estimated")
        ax1.plot(e_time, e_rx, color="cyan", label="rx_estimated")
        ...
        for f in ["png", "pdf"]:
            fig.savefig(pj(estimation_folder,
                           "series_estimation_%d_%s.%s" % (target, estimator, f)),
                        format=f)
...
def run(folder, reference):
    ...
    e.save_global(targets)

    for target in targets:
        e.check(target)
        e.save(target)
        plot_series(e.folder, target)
    plot_bin(e.folder)
...
if __name__ == '__main__':
    ...
    for folder in folders:
        # try:
        logging.basicConfig(filename="%s/estimator.log" %
                            folder, filemode="w", level=logging.DEBUG)
